...
class MySegment(nn.Module):
    def __init__(self, device:torch.device = None):
        super(MySegment, self).__init__()
        self.duration = 5.0
        self.step = 0.5
        self.batch_size = 32
    ...
    # from pyannote/audio/core/io.py
    def downmix_and_resample(self, waveform: torch.Tensor, sample_rate: int) -> torch.Tensor:
    ...
    def forward( self, waveform:torch.Tensor ):
        ...
        return segmentations # for testing
        ...
        masked_waveforms_batches = []

        for batch in batches:
            waveforms, masks = zip(*filter(lambda b: b[0] is not None, batch))

            # waveforms is tuple of tensors which is 1x1x80000
            #waveform_batch = torch.vstack(waveforms) --> changed to below
            # waveform_batch is tensor: 32x1x80000
            # (batch_size, 1, num_samples) torch.Tensor
            tmp = []
            for wf in waveforms:
                tmp.append( wf.numpy())
            waveform_batch = torch.from_numpy( np.vstack( tmp ))

            #mask_batch = torch.vstack(masks) --> changed to below
            # (batch_size, num_frames) torch.Tensor
            tmp = []
            for wf in masks:
                tmp.append( wf.numpy())
            mask_batch = torch.from_numpy( np.vstack( tmp ))

            masked_waveforms, wav_len = self.embedding_mask(
                waveform_batch, masks = mask_batch
            )
            # (batch_size, dimension) np.ndarray
            #masked_waveforms_batches.append( masked_waveforms.flatten() )
            masked_waveforms_batches.append( masked_waveforms )

        #xnew = torch.stack((masked_waveforms_batches[0], masked_waveforms_batches[0]), 0)

        return masked_waveforms_batches
        #return torch.from_numpy( np.array( aa ))
        #return xnew
